...
from .nestedsamplers import (UnitCubeSampler, SingleEllipsoidSampler,
                             ...
                             SupFriendsSampler, LivePointSampler)
from .utils import (get_seed_sequence, _kld_error,
                    compute_integrals,
                    get_enlarge_bootstrap, RunRecord, get_neff_from_logwt,
                    save_sampler, restore_sampler, Results)
...
wrong with your likelihood.''')
        zweight = xp.mean(len(logl))
    else:
        # TODO the logic here needs to be verified
        logz_remain = logl[-1] + logvol[-1]  # remainder
        logz_tot = xp.logaddexp(logz[-1], logz_remain)  # estimated upper bound
        lzones = xp.ones_like(logz)
        logzin = logsumexp([lzones * logz_tot, logz],
                           axis=0,
                           b=[lzones, -lzones])  # ln(remaining evidence)
        logzweight = logzin - xp.log(samples_n)  # ln(evidence weight)
        logzweight -= logsumexp(logzweight)  # normalize
        zweight = xp.exp(logzweight)  # convert to linear scale

    # Derive posterior weights.
    pweight = xp.exp(logwt - logz[-1])  # importance weight
    pweight /= xp.sum(pweight)  # normalize
...
def _initialize_live_points(live_points,
    ...
    if live_points is None:
        ...
        while True:
            ...
            cur_live_v = prior_transform(cur_live_u.T).T
...
class DynamicSampler:
    ...
    def __init__(self, loglikelihood, prior_transform, npdim, bound, method,
                 update_interval_ratio, first_update, rstate, queue_size,
                 ncdim, nlive0, kwargs):
    ...
    def sample_initial(self,
        ...
        maxiter : int, optional
            ...
            termination condition is reached. Default is `2**31 - 1`
            (no limit).
    ...
    def sample_batch(self,
        ...
        maxcall = maxcall or 2**31 - 1
        maxiter = maxiter or 2**31 - 1
    ...
    def run_nested(self,
        ...
        **The main dynamic nested sampling loop.** After an initial "baseline"
        ...
        maxcall_init : int, optional
            ...
            if the termination condition is reached. Default is `2**31 - 1`
            (no limit).
        ...
        maxiter : int, optional
            Maximum number of iterations allowed. Default is `2**31 - 1`
            (no limit).

        maxcall : int, optional
            Maximum number of likelihood evaluations allowed.
            Default is `2**31 - 1` (no limit).

        maxbatch : int, optional
            Maximum number of batches allowed. Default is `2**31 - 1`
            (no limit).
        ...
        if maxiter_batch is None:
            maxiter_batch = 2**31 - 1
        if maxcall_batch is None:
            maxcall_batch = 2**31 - 1
        if maxbatch is None:
            maxbatch = 2**31 - 1
