...
from dynesty.utils import get_nonbounded
...
def _get_citations(nested_type, bound, sampler):
    ...
    bound_refs = {
        ...
        [("Buchner (2016)", "ui.adsabs.harvard.edu/abs/2014arXiv1407.5459B"),
         ("Buchner (2017)", "ui.adsabs.harvard.edu/abs/2017arXiv170704476B")],
        'live':
        [("Gooman and Weare (2011)", ""), ("Veitch et al. (2014)", ""), ("Talbot et al. (2024)", "")],
    }
...
class NestedSampler(SuperSampler):
    ...
    def __new__(cls,
        ...
        update_interval = int(
            max(min(np.round(update_interval_ratio * nlive), 2**31 - 1), 1))
        ...
        if gradient is not None:
            grad = jax.grad(loglike.loglikelihood)
            jac = jax.grad(ptform)
...
class _function_wrapper:
    ...
    def pytree_flatten(self):
        return (self.func, self.args, self.kwargs, self.name), None

    @classmethod
    def pytree_unflatten(cls, aut_data, data):
        return cls(*data)
