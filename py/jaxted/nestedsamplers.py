...
from .sampling import (sample_unif, sample_slice, sample_rslice,
                       sample_hslice, fixed_rwalk_jax)
...
class LivePointSampler(UnitCubeSampler):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.n_accept = self.kwargs["walks"]
        self.kwargs["walks"] *= 4
    ...
    def update_rwalk(self, blob, update=True):
        ...
        # delay = self.nlive // 2
        delay = 2
        self.walks = (self.walks * delay + self.n_accept / accept_prob) / (delay + 1)
        self.kwargs["walks"] = min(int(np.ceil(self.walks)), self.n_accept * 1000)
